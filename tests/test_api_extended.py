...
def test_bulk_import_api():
    ...
    resp = client.post(
        ...
        json={"proxy_lines": proxy_lines, "group": "Test Group", "target_page": "google_login"},
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_warmup_plan_api():
    resp = client.post("/api/profiles/prof_01/warmup/plan", json={"niche": "tech", "steps_count": 3})
    assert resp.status_code == 200
    data = resp.json()
...
def test_cookies_import_api():
    ...
    resp = client.post("/api/profiles/prof_01/cookies/import", json={"cookies_data": cookies_data})
    assert resp.status_code == 200
