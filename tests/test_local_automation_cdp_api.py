...
from nazak.core.browser_launcher import BrowserLauncher, get_free_port
...
def test_dolphin_v1_start_stop_mocked():
    ...
    with patch.object(
        browser_launcher,
        "launch_with_cdp",
        return_value=(True, 9999, 9222, "ws://127.0.0.1:9222/devtools/browser/abc", None),
    ):
...
def test_nazak_api_v1_start_stop_endpoints():
    ...
    with patch.object(
        browser_launcher,
        "launch_with_cdp",
        return_value=(True, 8888, 9333, "ws://127.0.0.1:9333/devtools/browser/xyz", None),
    ):
