...
def test_api_create_profile_valid():
    payload = {
        ...
        "fingerprint": {"user_agent": "Mozilla/5.0", "screen_width": 1920, "screen_height": 1080},
    }
...
def test_api_bulk_import_proxies():
    ...
    res = client.post("/api/profiles/bulk-import", json={"proxy_lines": lines, "group": "Imported Test Group"})
    assert res.status_code == 200
    data = res.json()
...
def test_api_warmup_plan_ecommerce():
    res = client.post("/api/profiles/prof_01/warmup/plan", json={"niche": "ecommerce", "steps_count": 5})
    assert res.status_code == 200
    plan = res.json()
...
def test_api_warmup_plan_crypto():
    res = client.post("/api/profiles/prof_01/warmup/plan", json={"niche": "crypto", "steps_count": 3})
    assert res.status_code == 200
    plan = res.json()
...
def test_api_import_cookies_valid_json():
    ...
    res = client.post("/api/profiles/prof_01/cookies/import", json={"cookies_data": cookies_json})
    assert res.status_code == 200
    assert res.json()["parsed_cookies_count"] == 1


def test_api_import_cookies_invalid():
    res = client.post("/api/profiles/prof_01/cookies/import", json={"cookies_data": "not valid cookie data"})
    assert res.status_code == 400
...
def test_api_batch_stop():
    res = client.post("/api/profiles/batch-stop", json={"profile_ids": ["prof_01", "prof_02"]})
    assert res.status_code == 200
    assert res.json()["success"] is True
