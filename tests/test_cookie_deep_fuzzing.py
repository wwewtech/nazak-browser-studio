import pytest
import json
from nazak.core.cookie_manager import parse_netscape_cookies, cookies_to_netscape, parse_any_cookies
...
def test_cookies_to_netscape_format():
    cookies = [
        {"name": "SID", "value": "val1", "domain": ".google.com", "path": "/", "expires": 1750000000, "secure": True, "httpOnly": True},
        {"name": "NID", "value": "val2", "domain": "google.com", "path": "/search", "expires": 0, "secure": False, "httpOnly": False}
    ]
...
def test_cookies_to_netscape_and_back_roundtrip():
    original = [
        {"name": "cookieA", "value": "valA", "domain": ".site.com", "path": "/", "expires": 1750000000, "secure": True, "httpOnly": True}
    ]
...
def test_parse_any_cookies_json_array():
    json_data = json.dumps([
        {"name": "token", "value": "xyz123", "domain": "api.test.com", "path": "/", "expirationDate": 1750000000.5, "secure": True, "httpOnly": False}
    ])
...
def test_parse_any_cookies_normalizes_string_values():
    json_data = json.dumps([
        {"name": "num_val", "value": 12345, "domain": "test.com"}
    ])
    cookies = parse_any_cookies(json_data)
    assert len(cookies) == 1
...
def test_parse_any_cookies_boolean_conversions():
    json_data = json.dumps([
        {"name": "c1", "value": "v1", "secure": "true", "httpOnly": 1}
    ])
    cookies = parse_any_cookies(json_data)
    assert len(cookies) == 1
