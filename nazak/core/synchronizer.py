...
class SynchronizerSession:
    def __init__(self, master_profile_id: str, worker_profile_ids: List[str], humanize_jitter: bool = True, delay_range_ms: Tuple[int, int] = (20, 80), coordinate_jitter_px: int = 2):
...
def tile_windows_win32(pids: List[int], cols: Optional[int] = None) -> bool:
...
class SynchronizerManager:
    def __init__(self, browser_launcher):
        ...
        self.current_session: Optional[SynchronizerSession] = None

    def start_session(self, master_profile_id: str, worker_profile_ids: List[str], humanize_jitter: bool = True, delay_range_ms: Tuple[int, int] = (20, 80), coordinate_jitter_px: int = 2) -> SynchronizerSession:
        session = SynchronizerSession(master_profile_id=master_profile_id, worker_profile_ids=worker_profile_ids, humanize_jitter=humanize_jitter, delay_range_ms=delay_range_ms, coordinate_jitter_px=coordinate_jitter_px)
        session.active = True
        session.started_at = time.time()
        self.current_session = session
        return session

    def stop_session(self) -> Optional[SynchronizerSession]:
    ...
    def tile_active_windows(self, cols: Optional[int] = None) -> bool:
        pids = []
