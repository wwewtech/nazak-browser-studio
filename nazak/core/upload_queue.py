...
class UploadJob:
    ...
    profile_name: str
    source_video: str
    unique_video: Optional[str] = None
    title: str = ""
    description: str = ""
...
class UploadQueueManager:
    ...
    """
    def __init__(self, profile_manager, browser_launcher, ws_broadcast: Optional[Callable] = None):
    ...
    async def run_batch_upload(
        ...
        try:
            ...
            for idx, pid in enumerate(profile_ids, start=1):
                ...
                await self.broadcast("autopost_job_update", {"profile_id": pid, "status": job.status, "message": job.progress_message})
                ...
                if not ok or not out_path:
                    ...
                    await self.broadcast("autopost_job_update", {"profile_id": pid, "status": "failed", "error": job.error})
                    continue
                ...
                async def on_progress(msg: str):
                    ...
                    await self.broadcast("autopost_job_update", {"profile_id": pid, "status": job.status, "message": msg})
                ...
                if upload_ok:
                    ...
                    await self.broadcast("autopost_job_update", {
                        "profile_id": pid,
                        "status": "published",
                        "video_url": video_url,
