...
flowchart TD
    ...
    subgraph Hardware Isolation Shield
        C --> D1[GPU & WebGPU: RTX 4090 / 3080]
        C --> D2[Native Function Cloaking & Proxy toString]
        C --> D3[Canvas, OffscreenCanvas & WebAudio Noise]
        C --> D4[Local Fonts & SpeechSynthesis Matching]
        C --> D5[WebRTC Candidate Sanitizer & Timezone TZ]
        C --> D6[Organic Profile History Seeder SQLite]
    end
...
- `test_oauth_state_security.py` — OAuth security tests: single-use state tokens, TLS-only callback URL validation, HTML escaping, per-instance state isolation.
- `test_upload_queue_cdp_and_cancel.py` — dynamic CDP port allocation plus cancel-cleanup sweep (no leaked Chrome processes) and platform normalization.
- `test_server_bind_security.py` — GUI/API server defaults to loopback binding (`127.0.0.1`), with an explicit warning for `0.0.0.0`.
